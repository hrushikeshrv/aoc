...
# Set up the input
with open('inputs/input-04122020.txt', 'r') as file:
...
def get_passports(l):
    ...
    passports = []
    # We need a cache to keep track of when one passport ends and another one starts since they are inconsistently
    # separated by spaces and newlines.
    cache = ''
    
    for line in l:
        # When we encounter a new line, we end the previous passport here, add it to our list of passports and start
        # a new one
        if line == '\n':
...
def check_valid(d):
    ...
    try:
        ...
        # Else if the height is in cm and is not between 150 to 193 cm, this passport is invalid
            ...
            valid = False
        # Else if the height is not in cm and not in inches, this passport is invalid
